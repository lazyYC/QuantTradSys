...
from reporting.mean_reversion_report import (
    create_params_table,
...
def _load_equity(path: Path | None) -> pd.DataFrame:
    ...
    df = _filter_by_time(df, 'exit_time', start_ts, end_ts)
    df = _filter_by_time(df, 'created_at', start_ts, end_ts)
    return df
...
def _load_candles(path: Path, *, start_ts: pd.Timestamp | None = None, end_ts: pd.Timestamp | None = None) -> pd.DataFrame:
...
def _load_trades_from_db(args: argparse.Namespace, *, start_ts: pd.Timestamp | None = None, end_ts: pd.Timestamp | None = None) -> pd.DataFrame:
...
def _load_metrics_from_db(args: argparse.Namespace, *, start_ts: pd.Timestamp | None = None, end_ts: pd.Timestamp | None = None) -> pd.DataFrame:
...
def _filter_trades_window(trades: pd.DataFrame, start_ts: pd.Timestamp | None, end_ts: pd.Timestamp | None) -> pd.DataFrame:
    if trades.empty or 'entry_time' not in trades.columns:
        return trades
    filtered = trades.copy()
    filtered['entry_time'] = pd.to_datetime(filtered['entry_time'], utc=True, errors='coerce')
    filtered['exit_time'] = pd.to_datetime(filtered.get('exit_time'), utc=True, errors='coerce')
...
def _compute_window_metrics(candles: pd.DataFrame, equity_df: pd.DataFrame, trades: pd.DataFrame) -> pd.DataFrame:
    if candles.empty:
        return pd.DataFrame()
    ...
    if not equity.empty:
        ...
        annualized = (1 + total_return) ** (365 / window_days) - 1 if window_days > 0 else total_return
        ...
        metrics.update({'total_return': 0.0, 'annualized_return': 0.0, 'sharpe': 0.0, 'max_drawdown': 0.0})
...
def _collect_figures(
    ...
    if not candles.empty:
        ...
        if not trades.empty and {"entry_time", "exit_time"}.issubset(trades.columns):
            trimmed_trades = trades[(trades["exit_time"] >= start_ts) & (trades["entry_time"] <= end_ts)]
        if not equity_df.empty and {"timestamp"}.issubset(equity_df.columns):
            trimmed_equity = equity_df[(equity_df["timestamp"] >= start_ts) & (equity_df["timestamp"] <= end_ts)]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render mean reversion backtest report")
    ...
    parser.add_argument("--output", default="reports/mean_rev.html", help="輸出的 HTML 檔案路徑")
    parser.add_argument("--start", help="起始時間 (ISO，例如 2025-09-01 或 2025-09-01T00:00)")
    ...
    parser.add_argument("--params-db", default="storage/strategy_state.db", help="策略參數 SQLite (預設 storage/strategy_state.db)")
    parser.add_argument("--strategy", default="mean_reversion_optuna", help="策略名稱")
    parser.add_argument("--dataset", default="all", help="資料集標籤，例如 train/test/all")
...
def main() -> None:
    ...
    rankings_df = _load_rankings(Path(args.rankings_json)) if args.rankings_json else pd.DataFrame()
    equity_df = _load_equity(Path(args.equity_csv)) if args.equity_csv else pd.DataFrame()
    ...
    if metrics_df.empty and not rankings_df.empty:
        metric_cols = ["annualized_return", "total_return", "sharpe", "max_drawdown", "win_rate", "trades"]
        metrics_df = rankings_df.iloc[[0]][[col for col in metric_cols if col in rankings_df.columns]]
    if params_dict is None and not rankings_df.empty and "params" in rankings_df.columns:
