    ...
    if not trades.empty and not equity_curve.empty and not ohlcv.empty:
        ...
        if pd.notna(data_start) and pd.notna(data_end) and data_end > data_start:
            ...
            if period_days > 0:
                ...
                metrics["annualized_return"] = float((1 + total_geo_return) ** (365 / period_days) - 1)

                equity_series = equity_curve.set_index("timestamp")["equity"].sort_index()
                ...
                equity_series = pd.concat([baseline, equity_series]).sort_index().ffill()
