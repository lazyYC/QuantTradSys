...
class StarRuntimeState:
    ...
    def from_dict(cls, data: Optional[Dict[str, object]]) -> "StarRuntimeState":
        if not data:
            return cls()
        timestamp = pd.to_datetime(data.get("last_signal_timestamp"), utc=True, errors="coerce") if data.get("last_signal_timestamp") else None
        entry_ts = pd.to_datetime(data.get("entry_timestamp"), utc=True, errors="coerce") if data.get("entry_timestamp") else None
...
def _build_cache_for_runtime(df: pd.DataFrame, params: StarIndicatorParams) -> StarFeatureCache:
...
def generate_realtime_signal(
    ...
    if class_means_arr.size != len(CLASS_VALUES):
        padded = [class_means[i] if i < len(class_means) else 0.0 for i in range(len(CLASS_VALUES))]
    ...
    if runtime_state.position_side:
        ...
        if exit_signal and timestamp is not None and not np.isnan(price):
            if runtime_state.entry_price:
                if runtime_state.position_side == "LONG":
                    trade_return = (price - runtime_state.entry_price) / runtime_state.entry_price
                else:
                    trade_return = (runtime_state.entry_price - price) / runtime_state.entry_price
            else:
                trade_return = None
            context.update(
                {
    ...
    # 若無持倉，判斷是否滿足進場條件
    if runtime_state.position_side is None and expected_return >= threshold and not np.isnan(price):
        if pred_class == 2:
            context.update(
                {
                    "action": "enter_long",
...
__all__ = ["StarRuntimeState", "load_star_model", "generate_realtime_signal"]
