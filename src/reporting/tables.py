...
def _format_series_for_table(
    series: pd.Series, *, decimals: int = 4
...
def create_metrics_table(
    ...
    table = go.Table(
        ...
        cells=dict(
            values=[labels, values], fill_color="#f8fafc", align="left", height=28
        ),
    )
...
def create_params_table(
    ...
    table = go.Table(
        ...
        cells=dict(
            values=[labels, values], fill_color="#f1f5f9", align="left", height=28
        ),
    )
    fig = go.Figure(data=[table])
    layout_kwargs = _table_layout_settings(len(labels), title_present=bool(title))
...
def create_trade_distribution_table(
    ...
    table = go.Table(
        ...
        cells=dict(
            values=[labels, values], fill_color="#e2e8f0", align="left", height=28
        ),
    )
    fig = go.Figure(data=[table])
    layout_kwargs = _table_layout_settings(len(labels), title_present=bool(title))
...
def create_top_trades_table(
    ...
    closed["return"] = closed["return"].map(
    ...
    fig = go.Figure(data=[table])
    layout_kwargs = _table_layout_settings(
        len(closed), title_present=bool(title), min_height=320, max_height=640
    )
    fig.update_layout(title=title, template="plotly_white", **layout_kwargs)
    return fig
